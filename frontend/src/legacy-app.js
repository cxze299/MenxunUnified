...
const apiErrorMessages = {
  ...
  admin_reauthentication_required: '管理登录已超过 12 小时，请退出后重新登录再操作',
  checkin_save_failed: '该任务可能已经打卡，请刷新后确认',
  retro_window_expired: '已超过本组允许的补卡期限，请联系管理员补录',
  ...
  registration_review_failed: '审批失败，请稍后重试',
  rejection_reason_required: '请填写拒绝原因',
  username_exists: '该登录账号已被使用',
  email_exists: '该邮箱已被使用',
  roster_already_claimed: '该名单已经绑定账号',
...
export function createWeekDraft() {
  ...
  if (!latest) {
    ...
    const draft = weekDraftFromWeek(latest);
    const start = parseLocalDate(latest.end); start.setDate(start.getDate() + 1);
    const end = new Date(start); end.setDate(end.getDate() + 6);
    draft.id = 0;
